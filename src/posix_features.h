 ...
 * clock_gettime, and gethostname live in POSIX headers and stay hidden
 * unless a feature-test macro is set before any system include.
 *
 * macOS and MinGW headers are laxer, which is why those hosts compiled
 * without this. Linux did not.
 *
 * Only define these on Linux. _POSIX_C_SOURCE on Darwin can hide BSD
 * helpers (getifaddrs, getloadavg) that collect_darwin.c needs. */
